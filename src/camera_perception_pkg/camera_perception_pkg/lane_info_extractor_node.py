...
class Yolov8InfoExtractor(Node):
    ...
    def _get_line_endpoints_in_image(self, params, h_img, w_img):
        ...
        if abs(vy) > 1e-6:
            # 이미지 최상단(y=0)에서의 x 좌표
            x_top = x0 - (y0 * vx / vy)
            # 이미지 최하단(y=h_img-1)에서의 x 좌표
            x_bottom = x0 + ((h_img - 1 - y0) * vx / vy)

            # 좌표를 정수로 변환 + x 좌표가 이미지 밖이면 clip
    ...
    def _process_detections_and_visualize(self, detection_msg: DetectionArray, callback_id: str, src_mat_orig_ref):
        ...
        if all_lane_bird_image_combined is not None:
            ...
            if reference_roi_from_combined is not None and reference_roi_from_combined.size > 0:
                ...
                # 3. 기준 크기로 검은색 빈 이미지를 생성
                roi_image_for_laneinfo = np.zeros((h_ref_roi, w_ref_roi), dtype=np.uint8)
            else:
                self.get_logger().warn(f"detections_{callback_id}: Combined ROI for laneinfo is empty. Cannot determine size for RANSAC line drawing canvas.")
        else:
            self.get_logger().warn(f"detections_{callback_id}: Combined bird image for LaneInfo ROI reference is None.")

        # roi_image_for_laneinfo 생성 실패 시, 이후 로직에서 사용되지 않도록 None으로 두거나,
        # 또는 기본 크기의 검은 이미지로 설정할 수 있음 (현재는 None으로 두고 이후 방어 코드에서 처리)
        if roi_image_for_laneinfo is None:
             h_default_roi, w_default_roi = 100, 100 # 임의의 작은 크기
        ...
        # --- `roi_image_for_laneinfo` 생성 로직 종료 ---
        ...
        for type_name, edge_float_img, color, log_prefix in lane_types_to_process:
            ...
            if params_roi:
                h_roi_ind, w_roi_ind = individual_roi_image_uint8.shape[:2] # 개별 ROI의 크기
                ...
                if pt1_roi_ind and pt2_roi_ind:
                    ...
                    # 2. 검은색 roi_image_for_laneinfo에 그리기
                    # 가정: individual_roi_image_uint8와 roi_image_for_laneinfo의 ROI 공간이 일치 (동일한 BEV 및 cutting_idx 적용 결과)
                    # 즉, pt1_roi_ind, pt2_roi_ind 좌표를 roi_image_for_laneinfo에 직접 사용 가능
                    if roi_image_for_laneinfo is not None:
                        # roi_image_for_laneinfo의 크기와 individual_roi_image_uint8의 크기가 일치해야 함
                        # CPFL.roi_rectangle_below가 일관된 크기의 ROI를 반환한다고 가정
                        if roi_image_for_laneinfo.shape[0] == h_roi_ind and roi_image_for_laneinfo.shape[1] == w_roi_ind:
                             cv2.line(roi_image_for_laneinfo, pt1_roi_ind, pt2_roi_ind, 255, 1) # 흰색으로 그리기
                    
                        else:
                            self.get_logger().warn(f"{log_prefix}: Shape mismatch between individual ROI ({h_roi_ind},{w_roi_ind}) and combined ROI canvas ({roi_image_for_laneinfo.shape[:2]}). Line drawing on canvas might be inaccurate.")
                            # 대체 방안: params_roi와 roi_image_for_laneinfo의 크기를 사용하여 새로 끝점 계산
                            # pt1_draw, pt2_draw = self._get_line_endpoints_in_image(params_roi, roi_image_for_laneinfo.shape[0], roi_image_for_laneinfo.shape[1])
                            # if pt1_draw and pt2_draw:
                            #    cv2.line(roi_image_for_laneinfo, pt1_draw, pt2_draw, 255, 1)
                            # 간단하게는, 크기가 다르면 일단 그려보고 문제 발생 시 위 대체 방안 고려
        ...
        # LaneInfo 계산에는 이전에 생성한 all_lane_bird_image_combined (합쳐진 전체 에지의 BEV)를 사용하고,
        # roi_image_for_laneinfo는 이제 RANSAC 선이 그려진 검은색 이미지가 됨.
    ...
    def yolov8_detections_2_callback(self, detection_msg: DetectionArray):
        ...
        if self.show_image and roi_image_uint8_final is not None:
            ...
            for pt in target_points:
                cv2.circle(vis_image_with_points, (pt.target_x, pt.target_y), 3, (0, 0, 255), -1)  # 빨간 점
