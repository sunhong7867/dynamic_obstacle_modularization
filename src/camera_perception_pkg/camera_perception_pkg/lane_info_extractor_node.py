...
class Yolov8InfoExtractor(Node):
    ...
    def _process_detections_and_visualize(self, detection_msg: DetectionArray, callback_id: str, src_mat_orig_ref):
        ...
        for type_name, edge_float_img, color, log_prefix in lane_types_to_process:
            ...
            if params_roi:
                ...
                if pt1_roi_ind and pt2_roi_ind:
                    ...
                    if callback_id == "1":
                        # ── 실측 기준 ─────────────────────────────────────────────
                        # line(실선): BEV에서 오른쪽(x0 > center, 정상≈500)
                        # dotted_line(점선): BEV에서 왼쪽(x0 < center, 정상≈170)
                        # 반대 위치에 있으면 오염된 검출 → 버림
                        # ─────────────────────────────────────────────────────────
                        if type_name == "line" and x0_check < roi_center_x:
                            self.get_logger().warn(
                                f"[1] line x0({x0_check:.1f}) < 중앙({roi_center_x:.1f}): 위치 이상(왼쪽으로 튐), 버림")
                            continue
                        if type_name == "dotted_line" and x0_check > roi_center_x:
                            self.get_logger().warn(
                                f"[1] dotted_line x0({x0_check:.1f}) > 중앙({roi_center_x:.1f}): 위치 이상(오른쪽으로 튐), 버림")
                            continue
        ...
        if len(fitted_results) == 2:
            ...
            if callback_id == "1":
                # 정상: x0_line(오른쪽) > x0_dot(왼쪽)
                # 역전: x0_line <= x0_dot → line이 왼쪽으로 튄 것 → 버림
                if x0_line <= x0_dot:
                    self.get_logger().warn(f"[1] line 역전 감지 (line이 왼쪽으로 튐). 예측으로 대체.")
                    del fitted_results['line']
                # 폭 이상 감지: 정상 폭의 70% 미만이면 line 오염 의심
                ...
                # 정상: x0_dot(왼쪽) < x0_line(오른쪽)
                if x0_dot >= x0_line:
                    self.get_logger().warn(f"[2] dotted_line 역전 감지. 예측으로 대체.")
        ...
        if len(fitted_results) == 1:
            ...
            if callback_id == "1":
                # line: 오른쪽(x0≈500), dotted_line: 왼쪽(x0≈170)
                # line만 보이면 → 왼쪽으로 lane_width만큼 이동해서 dotted_line 예측
                # dotted_line만 보이면 → 오른쪽으로 lane_width만큼 이동해서 line 예측
